...
## Female maturation not used in assessment
if (FALSE){
  matp.aut_F <- 
    mfdb_sample_count(mdb, c('maturity_stage','age','length'),
                      append(defaults,
...
## Age-data used for benchmark but not assessment model
if (FALSE){
  aldist.bmt <-
    mfdb_sample_count(mdb,
                      c('age', 'length'),
...
if (defaults$save_bootstrap_data){
  save(ldist.aut, matp.aut, 
       #matp.aut_F,
       ldist.lln, ldist.bmt, ldist.comm, 
       #aldist.bmt, aldist.is, ldist.is, 
