...
penalise_recruitment <- 1
recruitment_sigma <- 0.4
...
vers <- paste0(vers, '_Linf', as.numeric(estimate_linf),
               '_K', as.numeric(estimate_k),
               '_t0', as.numeric(estimate_t0),
               '_penrec', penalise_recruitment,
               {if (penalise_recruitment == 1) paste0('_sigma', recruitment_sigma) else NULL},
...
year_range <- 1970:2023 #(lubridate::year(Sys.Date())-1)
project_years <- 100
species_name <- 'bli' 
...
si_len_slopes <- NULL   #c(20,52)                           # NULL to have slope = 1 for each component
...
profile_short <- FALSE
profile_reps <- 1
M_values <- seq(0.15, 0.25, by = 0.01)
...
vers <- paste0(vers, 
               '_bmtS', as.numeric(dome_bmt), 
               '_llnS', as.numeric(dome_lln),
               '_recl', as.numeric(mean_len_recl))
...
actions <- c(
  ...
  fleet_actions,
  likelihood_actions,
  if (penalise_recruitment == 1) random_actions else NULL,
...
tmb_param <- 
  ...
  g3_init_val('*.init.#', 10, lower = 0.001, upper = 100) |> #ifelse(random_initial, penalise_initial, 1)) |> 
  g3_init_val('*.rec.scalar', 20, spread = 0.99) |> 
  g3_init_val('*.init.scalar', 20, spread = 0.99) |>
  g3_init_val('*.init.F', 
                       ifelse((!initabun_by_age && !initabun_by_stock), 0.2, 0), 
                       spread = {if (!initabun_by_age && !initabun_by_stock) 0.75 else NULL}) |> 
  g3_init_val('*.lencv.#', lencv) |>   # Used to calculate length sd (in initial conditions)
  g3_init_val(paste0('*.lencv.', g3_stock_def(imm_stock, 'minage')), lencv, spread = 0.95) |>   # Used to calculate length sd (in initial conditions)
  g3_init_val('*.rec.sd', 5, lower = 0.1, upper = 10) |>     # To use lencv here, use g3a_renewal_normalcv instead of g3a_renewal_normalparam
  
  ## Growth
  g3_init_val('*.Linf', fixed_growth_pars$linf, lower = 90, upper = 170, optimise = estimate_linf) |> 
  g3_init_val(ifelse(timevarying_K, '*.K.#', '*.K'), fixed_growth_pars$k, lower = 0.04, upper = 0.2, optimise = estimate_k) |>
  g3_init_val('*.t0', -1, lower = -5, upper = 0, optimise = estimate_t0) |>
  g3_init_val('*.recl', 40, spread = 0.5) |>  #     Using t0 (g3a_renewal_vonb_t0) instead of recl (g3a_renewal_vonb_recl)
  g3_init_val('*.bbin', 100, lower = 1e-03, upper = 1000) |> 
  g3_init_val('*.M.#', 0.15) |> 
  
  ## Fleet selection parameters
  #  S-shaped
  g3_init_val('*.alpha', 0.1, lower = 0.01, upper = 2) |> 
  g3_init_val('*.l50', mean(do.call('c', sapply(stocks, g3_stock_def, 'midlen'))), spread = 0.75) |> 
  #  Dome-shaped
  g3_init_val('*.p1', log(2), lower = 0, upper = 3) |>
  g3_init_val('*.p3', 0.1, lower = 0.001, upper = 10) |>
  g3_init_val('*.p4', 0.02, lower = 0.001, upper = 1e3) |>
  ## Quota
  g3_init_val('*.quota_prop', 1, optimise = FALSE) |> 
  
  ## Maturity ogives
  #  For initial conditions if init_abund_mode = 1
  g3_init_val('*.mat_initial_alpha', 0.5, lower = 0.01, upper = 3) |> 
  g3_init_val('*.mat_initial_a50', 10) |> 
  #  For maturation
  g3_init_val('*.mat.alpha', 0.07, lower = 0.001, upper = 0.2) |>
  g3_init_val('*.mat.l50', mat.l50$l50, spread = 0.8) |> 
...
#gadgetplots::gadget_plots(g3_fit(model, tmb_param), file_type = 'html', path = 'tmp2')
