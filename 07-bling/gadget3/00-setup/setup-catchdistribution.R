...
minage <- min(sapply(stocks, gadget3::g3_stock_def, 'minage'))
maxage <- max(sapply(stocks, gadget3::g3_stock_def, 'maxage'))
minlen <- min(sapply(stocks, gadget3::g3_stock_def, 'minlen'))
...
ldist.aut <-
  mfdb::mfdb_sample_count(mdb, 
                          ...
                          c(list(
                            ...
                            sampling_type = 'AUT',
                            age = mfdb::mfdb_interval("all",c(minage,maxage),
                                                      open_ended = c("upper","lower")),
                            length = mfdb::mfdb_interval("len", 
...
## Maturity
matp.aut <- 
  mfdb::mfdb_sample_count(mdb, c('maturity_stage','age','length'),
                          append(defaults,
                                 list(sampling_type='AUT',
                                      ...
                                      maturity_stage = mfdb::mfdb_group(blingimm = 1, blingmat = 2:5))))
...
aldist.bmt <-
  mfdb::mfdb_sample_count(mdb,
                          ...
                          c(list(sampling_type = 'SEA',
                                 ...
                                 gear=c('BMT','NPT','DSE','PSE','PGT','SHT','GIL'),
                                 age = mfdb::mfdb_interval('age',c(minage:maxage),open_ended = c('upper')),
                                 length = mfdb::mfdb_interval("len",
                                                              seq(minlen, maxlen, by = dl),
                                                              open_ended = c("upper","lower"))),
                            defaults))
